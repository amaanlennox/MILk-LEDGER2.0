    ...
    <div className="container max-w-3xl mx-auto p-4 sm:p-6 page-transition">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 bg-slate-900/40 border border-slate-800/40 p-4 rounded-2xl backdrop-blur-md">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">{t('customerManagement')}</h1>
        </div>
      ...
      <Card className="glass-card border border-slate-800/60 shadow-xl rounded-2xl">
        <CardHeader className="p-6 pb-4">
          <CardTitle className="text-xl font-black text-white/90">{t('customers')}</CardTitle>
          <CardDescription className="text-xs text-muted-foreground font-medium">
            {t('manageYourCustomers')}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 pt-0">
          <div className="space-y-3">
            {customers.map((customer) => (
              <div key={customer.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-800/60 bg-slate-950/40 hover:bg-slate-900/30 transition-all duration-200">
                <div>
                  <p className="font-extrabold text-base text-slate-100">{customer.name}</p>
                ...
                <AlertDialog>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="bg-slate-900 border-slate-800 text-slate-200">
                      <DropdownMenuItem onClick={() => handleEdit(customer)} className="text-sm p-2.5 focus:bg-slate-800 focus:text-white cursor-pointer">
                  ...
                  <AlertDialogContent className="bg-slate-900 border-slate-800 text-slate-200">
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={() => deleteCustomer(customer.id)} className="bg-rose-600 text-white hover:bg-rose-700 font-bold">
                        {t('delete')}
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
             ...
             {customers.length === 0 && <p className="text-muted-foreground text-center p-10 text-sm">{t('noCustomers')}</p>}
          </div>
        </CardContent>
      </Card>
