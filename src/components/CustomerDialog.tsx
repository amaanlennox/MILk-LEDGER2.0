  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto bg-slate-900/95 border border-slate-800/80 text-slate-100 backdrop-blur-xl rounded-2xl shadow-2xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-black text-white">{customer ? t('editCustomer') : t('addCustomer')}</DialogTitle>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">{t('customerName')}</FormLabel>
                  <FormControl>
                    <Input placeholder={t('customerName')} {...field} className="bg-slate-950/50 border-slate-800 text-white rounded-xl h-10 focus-visible:ring-1 focus-visible:ring-indigo-500" />
                  </FormControl>
            ...
            {(watchedMilkTypes || []).includes('buffalo') && (
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <Input type="number" step="0.01" placeholder="60" {...field} value={field.value ?? 0} className="bg-slate-950/50 border-slate-800 text-white rounded-xl h-10 focus-visible:ring-1 focus-visible:ring-indigo-500" />
                        </FormControl>
                        <FormMessage className="text-rose-400 text-xs font-bold" />
                        </FormItem>
                    )}
